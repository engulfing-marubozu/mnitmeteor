    ...
    if (check) {
     ...
     //   console.log(token);
        ...
        jwt.verify(token, process.env.JWT_SECRET, async (err, user) => {
            ...
            if(hs>0){
                console.log("block hua ");
                var time_remaining = await redis.ttl(admin.email);
            ...
            bcrypt.compare(unicode, process.env.UNICODE, (err, data) => {
                ...
                const redisHandler = async (email, res) => {
                    t = (await redis.get(email));
                    console.log(t);
                    const hits = await redis.incr(email);

                    if (hits > 2) {
                        await redis.set(email, -1);
                        const blockingTime = 60*60*12;
                        var time_block = timeConvert(blockingTime);

                        await redis.expire(email, blockingTime);
                         //40 seconds 
...
const admin_verification_t = async (req, res, next) => {
    try {
        await redis.connect();
    } catch (error) {
        console.log("Cannot connect to client");
